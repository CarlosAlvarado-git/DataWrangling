...
# Dia del último eclipse
Dia_eclipse
# Valor de Saros
Saros
Fecha_final <- Dia_eclipse + Saros
# Fecha de predicción de eclipse: Dia_eclipse + Saros
Fecha_final
```
...
data %>%
  group_by(duration) %>%
  summarise(Cantidad_de_llamadas = n())
```

# Parte 3: Signo Zodiacal
Funcion: utiliza interval para crear el intervalo que se encuentra el signo Zodiacal
```{r}
...
(valor)
valor <- signo(ymd("2017-04-24"))
(valor)
valor <- signo(ymd("2017-07-24"))
(valor)
```
...
vuelos %>%
  select(dep_time_final, sched_dep_time_final, arr_time_final, sched_arr_time_final, delay_salida, delay_llegada, delay_total)
```
